...
\textbf{БАГ, НАЙДЕН И ПОЧИНЕН 2026-09-24: до фикса формула выше была
только ЗАМЫСЛОМ, а развёрнутая в ClickHouse VIEW считала другое.} В
\texttt{\seqsplit{rolling\_m}}
(\texttt{\seqsplit{schema\_realt\_metrics\_views.sql}}) было написано
\code{SELECT acquisition_month AS month, countIf(month >= acquisition_month AND month < acquisition_month + INTERVAL 3 MONTH)}
--- внутри \texttt{countIf} имя \texttt{month} разрешалось в
\textbf{алиас} (то есть в \texttt{\seqsplit{acquisition\_month}}), а не в
исходную колонку месяца визита. Условие вырождалось в тождество
«\,$M \ge M$ и $M < M{+}3$\,» = всегда истина, и окно не применялось вовсе:
фактически считались ВСЕ визиты когорты за всю загруженную историю.
Проверено численно --- колонка VIEW совпадала с
\code{count()} без условия во все 20 месяцев 2025--2026 (январь 2026: VIEW
давала 567 = все визиты когорты, честное окно --- 436). У самых свежих
когорт числа совпадают случайно (будущих визитов ещё нет в базе), поэтому
баг и не был виден два месяца: июнь--август 2026 сходились, январь--май
были завышены на 20--30\,\%.

\textbf{Фикс} (согласован с владельцем 2026-09-24, накатан на прод):
алиас переименован в \texttt{\seqsplit{cohort\_month}}, в финальном SELECT
\code{r.month} заменён на \code{r.cohort_month} (в
\texttt{coalesce} и в \code{FULL OUTER JOIN ... ON}); сам \texttt{countIf}
не менялся. Что стало со строкой «Визиты на нового клиента (3 мес,
скользящее)» карточки 186:
...
Когорта & Было & Стало & \\
\midrule
Янв 2026 & 2{,}81 & 2{,}16 & завышение $+30\,\%$ \\
Фев 2026 & 2{,}29 & 1{,}88 & $+22\,\%$ \\
Мар 2026 & 2{,}08 & 1{,}74 & $+20\,\%$ \\
Июн 2026 & 1{,}86 & 1{,}86 & без изменений \\
Авг 2026 & 1{,}29 & 1{,}29 & без изменений \\
«За год» 2026 & 2{,}24 & 1{,}80 & \\
\bottomrule
\end{tabular}
\normalsize\end{center}

\texttt{\seqsplit{new\_clients}} (знаменатель) фикс не затронул. Кривая
метрики по-прежнему падающая, но слабее: $2{,}16 \to 1{,}29$ вместо
$2{,}81 \to 1{,}29$ --- часть прежнего «падения» была артефактом бага (у
старых месяцев в числителе лежали лишние визиты, у свежих их физически
нет). \textbf{Общий вывод на будущее:} не давать колонке алиас с именем
другой колонки того же скоупа --- ClickHouse при разборе выражений SELECT
ищет имя сначала среди алиасов, ошибки не будет, условие просто молча
станет тождеством.
...
\textbf{Раскладка} (переделана в тот же день по просьбе владельца ---
«сделать как сводную, не так страшно»). Отдельной колонки «Тип» нет:
перед каждой группой идёт строка-заголовок с названием капсом
(«ПСИХИАТРЫ»\,/\,«ПСИХОЛОГИ»\,/\,«ПРОЧИЕ»), подсвеченная фоном через
\texttt{\seqsplit{table.column\_formatting}} (три правила
\texttt{\seqsplit{highlight\_row}} по точному совпадению текста в колонке
«Врач» --- меняя эти подписи в SQL, надо править и
\texttt{\seqsplit{visualization\_settings}} карточки). Врачи под
заголовком --- просто по фамилии, в порядке убывания числа новых
клиентов за год.

В строке-заголовке метрической карточки 196 месячные значения и «За год»
\textbf{пустые сознательно}, заполнена только «Новых клиентов за год»
(обычная сумма). Причина та же, по которой убрана строка «ВСЯ КЛИНИКА»:
в числителе только визиты к СВОЕМУ врачу, поэтому клиент, перешедший от
одного психиатра к другому, в «среднем по группе» всё равно не учтён ---
такое среднее вводило бы в заблуждение. В карточке 197 (счётчики
клиентов) строка-заголовок, наоборот, заполнена: счётчики складываются
корректно, и сумма по группам сходится с общим числом новых клиентов
месяца.

\textbf{Градиент и разница «0» / пусто в карточке 197} (добавлено
2026-09-24). Месячные колонки и «За год» карточки 197 покрашены
градиентом красный\,--\,жёлтый\,--\,зелёный. Шкала задана
\textbf{фиксированной} (\texttt{\seqsplit{min\_type}}\,/\,\texttt{\seqsplit{max\_type}}
= \code{custom}: месяцы $0\ldots25$, «За год» $0\ldots120$), а не «по всем
значениям» --- иначе строки-заголовки групп (до 199 клиентов в месяце)
растянули бы шкалу, и все врачи (месячные значения $0\ldots47$, p90 без
нулей $13\ldots27$ по разным годам) слиплись бы в красном конце.

При этом «0» и пустая ячейка означают разное и \textbf{не должны}
краситься одинаково: «0» --- месяц есть в данных, но у врача не было ни
одного нового клиента; пусто --- месяца ещё нет в данных вовсе (например
сентябрь--декабрь 2026 при загрузке по август). Поэтому в карточке 197
незагруженные месяцы отдаются как NULL: CTE \texttt{\seqsplit{loaded}}
собирает месяцы, в которых есть хотя бы один новый клиент по всей
клинике, и колонка месяца вне этого набора зануляется в NULL. Без этого
пустые месяцы красились бы «плохим» красным наравне с настоящими нулями,
то есть таблица врала бы цветом. В метрической карточке 196 такой маски
не нужно --- там знаменатель проходит через \code{nullIf} и
незагруженные месяцы и так выходят пустыми.

\textbf{Почему это НЕ визуал «Сводная таблица» Metabase.} Проверено
эмпирически 2026-09-24 на пробной карточке: в Metabase 0.63.10
\code{display: "pivot"} на native SQL-карточке ломает результат ---
появляется служебная колонка \texttt{\seqsplit{pivot-grouping}}, а
строки приходят вида \code{[0]}; тот же запрос с \code{display: "table"}
отдаёт нормальные данные, бэкендный
\code{/api/card/:id/query/pivot} на такой карточке --- 404. Это та же
граница, из-за которой карточка 186 в своё время была пересобрана из
GUI-question в native SQL с ручным разворотом месяцев в столбцы
(см.\ раздел~\ref{sec:monthly}). Поэтому «сводный» вид здесь собран
вручную в самом SQL.
