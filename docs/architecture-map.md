...
**Осознанное исключение из правила «на дашборде только Визуалы» (найдено и задокументировано 2026-09-19, `.claude/knowledge/architecture-standarts.md`).** Все 4 дашборда Реальта (id 5/6/7/8) кладут на дашборд карточку-Таблицу (`display: table`) напрямую, а не Визуал/MBQL-пивот — в отличие от CloudSix (дашборд id 2), где на дашборде только Визуалы, как и требует правило. Причина не в небрежности: в этих таблицах есть ratio-строки (например, «Средний чек», «Визиты на нового клиента (3 мес.)» в карточке 186), у которых столбец «За год» — не сумма месячных значений, а `sum(числитель)/sum(знаменателя)`. MBQL-пивот считает «Итого» той же агрегацией, что и ячейки (`sum`), и не умеет применить другую формулу для конкретной строки-метрики — поэтому пивот в принципе не может выразить такую таблицу правильно, и native SQL Таблица здесь — не обход процесса, а единственный работающий вариант. Подробности формулы и оговорка «не суммировать по месяцам» — `docs/formulas/realt.tex`, раздел «"За год" для ratio-метрик».
