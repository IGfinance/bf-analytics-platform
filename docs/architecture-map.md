...
| Себестоимость по неделям (.xlsx) | `ingest_wb_cogs.py` | `wb_cogs_core.ingest_file` | `wb_cogs_weekly` |
...
```mermaid
flowchart TD
    subgraph ADMIN_CS["CloudSix | Админка (id 11) — 16 карточек"]
        M49["Модель 49<br/><small>WB / кабинет·месяц<br/>+ себестоимость</small>"]
        M62["Модель 62<br/><small>Ozon / кабинет·месяц<br/>+ себестоимость</small>"]
        M191["Модель 191<br/><small>Ozon / кабинет·артикул·месяц<br/>+ себестоимость</small>"]
        M90["Модель 90<br/><small>WB / кабинет·артикул·месяц<br/>+ себестоимость</small>"]
        DASH2["Дашборд 2<br/><small>«01 Адаптеры»<br/>плоская выгрузка для адаптера</small>"]
        DASH4["Дашборд 4<br/><small>«02 Отчёт по артикулам Wb»<br/>строки 16-17</small>"]
        DASH11["Дашборд 11<br/><small>«03 Отчёт по артикулам Ozon»<br/>новый, 2026-09-23</small>"]
    end
...
**Переименования 2026-09-23.** Поартикульные карточки заняли имена «Полный отчет WB/Ozon» (показатели × месяцы по одному артикулу) и «Отчет по артикулам WB/Ozon» (артикулы × показатели за период), а прежние кабинетные визуалы 43/64 стали «Отчет для адаптера WB/Ozon» и переделаны из пивота в плоскую широкую выгрузку (месяц, кабинет, площадка + показатели 01…17 по столбцам, см. `src/metabase_queries/*_adapter_report.sql`). Побочный эффект: unpivot-таблицы 42 и 63 остались без потребителей — визуалы теперь читают Модели 49/62 напрямую; решение по ним не принято. Карточка 91 («Таблица - Полный отчет Wb по артикулу», wide-формат) заархивирована как никем не используемая.
