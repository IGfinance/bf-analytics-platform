...
- Кнопки primary: `bg-accent hover:bg-accent-hover text-white rounded-lg px-4 py-2 text-sm font-medium` — `text-sm` обязателен (без него кнопка наследует размер тела страницы, а не строку `text-sm font-medium` типографической шкалы, и визуально выбивается из текста рядом)
- Кнопки secondary: `border border-[--border] hover:border-[--border-hover] text-primary rounded-lg px-4 py-2 text-sm`
...
(Дашборд, Загрузка); выбор сразу переходит на ту же страницу другого
проекта (`data-project-select` в `shell.js`). Если у пользователя доступ
только к одному проекту, поле не скрывается — остаётся на месте,
`disabled` и серое (см. «Единая карточка недоступного источника» ниже —
тот же принцип: место в вёрстке не пропадает, меняется только состояние).
На Дашборде — рядом с h1 (`flex justify-between`); на Загрузке — в общей
карточке с полем «Кабинет», проект левее.
...
Все карточки на `/p/<slug>/upload` (WB детальный/сводный отчёт, площадки
без адаптера вроде Ozon, источники банк/карты/Клиентикс/Google-Таблицы) —
один и тот же partial `partials/upload_card.html`: заголовок (h2),
описание фиксированной высоты (`min-h-[2.5rem]`, чтобы карточки
выравнивались независимо от длины текста), затем либо форма (файл + кнопка
«Загрузить», единый текст без вариаций вроде «Загрузить и сверить»), либо
`card_disabled_reason`, если источник/площадка недоступны этому проекту.
Кабинет — одно общее поле над карточками (`partials/cabinet_field.html`,
`data-cabinet-shared`), значение зеркалится в скрытые поля каждой формы
через `shell.js` (`data-cabinet-mirror`), а не дублируется в каждой
карточке отдельно.

**Единая карточка недоступного источника/площадки**

Страница загрузки показывает ВЕСЬ каталог площадок и источников всегда
(одинаковый набор карточек у каждого проекта), а не только то, что
включено у конкретного клиента — иначе набор карточек скачет от проекта
к проекту и трудно понять, чего не хватает. Недоступная карточка
(`card_disabled=true` в `upload_card.html`) — `opacity-50` на всей
карточке, никакой формы/поля/кнопки внутри (не просто `disabled`-атрибут
на кнопке — кнопки нет вовсе), вместо формы — `card_disabled_reason`
(`text-xs text-secondary`), поясняющий почему («У проекта нет кабинетов
WB.», «Источник недоступен для этого проекта.», «Площадка пока не
поддерживается — адаптер загрузки ещё не реализован.»). Ровно одна и та
же карточка/классы для площадок (WB/Ozon) и источников (банк/карты/
Клиентикс/Google-Таблицы) — не два разных стиля «недоступности».
