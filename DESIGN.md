...
(`/p/<slug>/`, включает состояние сверки — отдельной страницы «Тесты»
больше нет), **Загрузка** (`/p/<slug>/upload`), **Профиль** (`/profile`).
...
«Проекты» или выбора в переключателе.

```
┌──────────────────────────────────────────────────────────────────┐
│ лого · Проекты · Дашборд · Загрузка · Профиль   [проект ▾] 🔔 email │ bg-surface, border-b
├──────────────────────────────────────────────────────────────────┤
...
На мобильном/планшете (`< 1024px`) пункты навигации сворачиваются в
бургер-меню справа в шапке, раскрываются вертикальным списком под
шапкой (см. `partials/header.html` + `data-mobile-nav-toggle` в
`shell.js`). Переключатель проекта — dropdown справа в шапке, список —
все проекты, доступные пользователю (`user_projects`).
