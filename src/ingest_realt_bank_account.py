#!/usr/bin/env python3
"""
Загрузка «Расчетного счета» Реальта (вкладка Google-Таблицы) в ClickHouse —
CLI-обёртка над realt_gsheets_core.ingest_bank_account. Данные тянутся
напрямую через Google Sheets API (сервис-аккаунт), файл выгружать не нужно.

Пример:
    python3 ingest_realt_bank_account.py --project-id 2
    python3 ingest_realt_bank_account.py --project-id 2 --dry-run
"""
...
from realt_gsheets_core import (
    SCRIPT_DIR, BANK_ACCOUNT_SHEET_NAME, read_tab, parse_bank_account, ingest_bank_account,
)
...
def main():
    parser = argparse.ArgumentParser(description="Загрузка «Расчетного счета» Реальта из Google-Таблицы в ClickHouse")
    ...
    if args.dry_run:
        values = read_tab(spreadsheet_id, BANK_ACCOUNT_SHEET_NAME)
        rows, skipped = parse_bank_account(values)
        print(f"Строк «Расчетного счета»: {len(rows)}, пропущено: {skipped}")
        print("Dry-run: в ClickHouse ничего не пишу.")
        return

    try:
        summary = ingest_bank_account(args.project_id, spreadsheet_id=spreadsheet_id)
