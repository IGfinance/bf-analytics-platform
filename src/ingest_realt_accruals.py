#!/usr/bin/env python3
"""
Загрузка «Начислений» Реальта (вкладка Google-Таблицы) в ClickHouse —
CLI-обёртка над realt_gsheets_core.ingest_accruals. Данные тянутся напрямую
через Google Sheets API (сервис-аккаунт), файл выгружать не нужно.

Пример:
    python3 ingest_realt_accruals.py --project-id 2
    python3 ingest_realt_accruals.py --project-id 2 --dry-run
"""
...
from realt_gsheets_core import (
    SCRIPT_DIR, ACCRUALS_SHEET_NAME, read_tab, parse_accruals, ingest_accruals,
)
...
def main():
    parser = argparse.ArgumentParser(description="Загрузка «Начислений» Реальта из Google-Таблицы в ClickHouse")
    ...
    if args.dry_run:
        values = read_tab(spreadsheet_id, ACCRUALS_SHEET_NAME)
        rows, skipped = parse_accruals(values)
        print(f"Строк «Начислений»: {len(rows)}, пропущено: {skipped}")
        print("Dry-run: в ClickHouse ничего не пишу.")
        return

    try:
        summary = ingest_accruals(args.project_id, spreadsheet_id=spreadsheet_id)
