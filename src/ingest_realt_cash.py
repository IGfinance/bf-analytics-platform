#!/usr/bin/env python3
"""
Загрузка «Наличных» Реальта (вкладка Google-Таблицы) в ClickHouse —
CLI-обёртка над realt_gsheets_core.ingest_cash. Данные тянутся напрямую
через Google Sheets API (сервис-аккаунт), файл выгружать не нужно.

Пример:
    python3 ingest_realt_cash.py --project-id 2
    python3 ingest_realt_cash.py --project-id 2 --dry-run
"""
...
from realt_gsheets_core import (
    SCRIPT_DIR, CASH_SHEET_NAME, read_tab, parse_cash, ingest_cash,
)
...
def main():
    parser = argparse.ArgumentParser(description="Загрузка «Наличных» Реальта из Google-Таблицы в ClickHouse")
    ...
    if args.dry_run:
        values = read_tab(spreadsheet_id, CASH_SHEET_NAME)
        rows, skipped = parse_cash(values)
        print(f"Строк «Наличных»: {len(rows)}, пропущено: {skipped}")
        print("Dry-run: в ClickHouse ничего не пишу.")
        return

    try:
        summary = ingest_cash(args.project_id, spreadsheet_id=spreadsheet_id)
