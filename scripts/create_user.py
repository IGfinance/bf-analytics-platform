...
Пример:
    python3 scripts/create_user.py --email ilya@finance-black.ru --project cloudsix
    python3 scripts/create_user.py --email new@finance-black.ru   # без проекта
"""
...
def create_user(client, email: str, password: str) -> int:
    existing = client.query(
        "SELECT count() FROM users WHERE email = {email:String}",
...
def main():
    ...
    try:
        user_id = create_user(client, args.email, password)
